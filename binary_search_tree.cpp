...
void binary_search_tree::print_pre_order(void){
	if(root == NULL)
        cout << "Empty tree, pre order print will be canceled \n";
	else{
...
bool binary_search_tree:: rec_delete_data(binary_node **pt, int key){
	if (*pt == NULL){
        cout << "The number " << key << " could not be deleted\n";
        return false;
    }

	if ((*pt)->data > key)
        return rec_delete_data(&((*pt)->left), key);
	else if ((*pt)->data < key)
        return rec_delete_data(&((*pt)->right), key);
	else if ((*pt)->data == key){
		if ((*pt)->left == NULL)
            *pt = (*pt)->right;
		else if ((*pt)->right == NULL)
            *pt = (*pt)->left;
		else if((*pt)->left->right == NULL){
		    (*pt)->left->right = (*pt)->right ;
            *pt = (*pt)->left ;
        }
		else if((*pt)->right->left == NULL){
